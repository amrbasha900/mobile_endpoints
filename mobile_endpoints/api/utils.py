...
def get_customer(page: int | str = 1, page_size: int | str = 20, search: str | None = None):
	...
	customers = [
		{"id": r.get("id") or r.get("name"), "name": r.get("name") or r.get("customer_name") or r.get("id")}
		for r in rows
	]
...
def get_items(page: int | str = 1, page_size: int | str = 20, search: str | None = None):
	...
	items = [
		{"id": r.get("id") or r.get("name"), "name": r.get("name") or r.get("id"), "price": 0} for r in rows
	]
